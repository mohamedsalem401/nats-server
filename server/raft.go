...
func (n *raft) notActiveLease() bool {
	...
	return time.Since(n.active) > (leaseTimeout() - hbInterval)
}
...
func (n *raft) applyCommit(index uint64) error {
	...
	// Since we've committed this entry, extend our leader lease to when we created this entry.
...
CONTINUE:
	...
	if sub != nil && isNew {
		...
		// Refresh our knowledge of the leader lease.
		if ae.time.After(n.lease) {
			n.lease = ae.time
		}
	}
...
func (n *raft) switchToCandidate() {
	...
	// If we're aware of an active leader lease, wait for it to expire (unless we're transferring leaders).
	if n.observer || n.paused || n.processed < n.commit || (!n.lxfer && time.Since(n.lease) <= leaseTimeout()) {
